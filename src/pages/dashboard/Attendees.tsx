...
const statusConfig: Record<string, { icon: typeof CheckCircle2; color: string; label: string }> = {
  confirmed: { icon: CheckCircle2, color: "text-teal", label: "Confirmed" },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="font-heading text-xl font-800 text-foreground">Attendees</h1>
          <p className="text-muted-foreground text-sm font-body mt-1">Manage registrations across all events</p>
        </div>
        <Button onClick={handleExport} disabled={filtered.length === 0} variant="ghost" className="border border-border text-foreground hover:bg-card font-heading font-700 text-sm">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-card rounded-xl border border-border p-4 text-center">
          <div className="font-heading text-2xl font-800 text-amber">{totalAttendees}</div>
          <div className="text-muted-foreground text-xs font-body mt-1">Total</div>
        </div>
        <div className="bg-card rounded-xl border border-border p-4 text-center">
          <div className="font-heading text-2xl font-800 text-teal">{confirmed}</div>
          <div className="text-muted-foreground text-xs font-body mt-1">Confirmed</div>
        </div>
        <div className="bg-card rounded-xl border border-border p-4 text-center">
          <div className="font-heading text-2xl font-800 text-electric">{checkedIn}</div>
          <div className="text-muted-foreground text-xs font-body mt-1">Checked In</div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {["all", "confirmed", "pending", "cancelled"].map((s) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-heading font-700 capitalize whitespace-nowrap transition-colors ${
                statusFilter === s ? "bg-amber text-ink" : "bg-card border border-border text-muted-foreground hover:text-foreground"
              }`}
            >
              {s}
            </button>
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-heading font-700 whitespace-nowrap transition-colors ${
              checkinFilter !== "all" ? "bg-teal text-ink" : "bg-card border border-border text-muted-foreground hover:text-foreground"
            }`}
          >
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          ...
          <h3 className="font-heading text-lg font-700 text-foreground mb-1">No attendees found</h3>
          <p className="text-muted-foreground text-sm font-body">Registrations will appear here once people sign up for your events</p>
        </div>
      ) : (
            ...
            return (
              <div key={reg.id} className="bg-card rounded-xl border border-border p-4 flex items-center gap-4">
                ...
                <div className="w-10 h-10 rounded-full bg-amber/10 flex items-center justify-center shrink-0">
                  <span className="font-heading text-sm font-700 text-amber">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-heading text-sm font-700 text-foreground truncate">{reg.full_name}</div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {reg.checked_in && (
                    <span className="px-2 py-0.5 rounded-full bg-teal/10 text-teal text-[10px] font-heading font-700 uppercase tracking-wider">
                      In
                    </span>
                  )}
