...
const Overview = () => {
  ...
  const statCards = [
    ...
    { label: "Total Attendees", value: stats.totalAttendees.toLocaleString(), icon: Users, color: "text-emerald-500" },
    { label: "Revenue", value: `₦${stats.totalRevenue.toLocaleString()}`, icon: Wallet, color: "text-primary" },
    { label: "Check-ins", value: stats.totalCheckins.toLocaleString(), icon: ScanLine, color: "text-emerald-500" },
  ];

  return (
    <div className="space-y-6">
      <h1 className="font-heading text-xl font-extrabold text-foreground">Overview</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {statCards.map((s) => (
          <div key={s.label} className="p-4 rounded-xl bg-card border border-border">
            ...
            <div className={`font-heading text-lg md:text-xl font-extrabold ${s.color} truncate`}>{s.value}</div>
            <div className="text-muted-foreground text-xs font-medium mt-0.5">{s.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-xl bg-card border border-border p-5">
          <h2 className="font-heading text-sm font-bold text-foreground mb-4">Recent Registrations</h2>
              ...
              recentRegistrations.map((r, i) => (
                <div key={i} className="flex items-center justify-between border-b border-border/50 pb-3 last:border-0 last:pb-0">
                  <div className="min-w-0 flex-1">
                    <div className="text-foreground text-sm font-heading font-bold truncate">{r.full_name}</div>
                    <div className="text-muted-foreground text-xs font-medium truncate">{(r.events as any)?.title}</div>
                  </div>
        ...
        {/* Upcoming Events */}
        <div className="rounded-xl bg-card border border-border p-5">
          <h2 className="font-heading text-sm font-bold text-foreground mb-4">Your Recent Events</h2>
          <div className="space-y-4">
            {upcomingEvents.length === 0 ? (
              ...
              upcomingEvents.map((e, i) => {
                ...
                return (
                  <div key={i} className="flex items-center justify-between border-b border-border/50 pb-3 last:border-0 last:pb-0">
                    <div className="min-w-0 flex-1">
                      <div className="text-foreground text-sm font-heading font-bold truncate">{e.title}</div>
                      <div className="text-muted-foreground text-xs font-medium">{new Date(e.date).toLocaleDateString()}</div>
                    </div>
                    <div className="text-right shrink-0 ml-2 w-24">
                      <div className="text-primary text-xs font-heading font-bold mb-1">{totalSold}/{totalTickets}</div>
